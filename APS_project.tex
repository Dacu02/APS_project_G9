...
\newline La crittografia asimmetrica prevede l'utilizzo di una coppia di chiavi da parte di ciascun attore: una chiave pubblica, che viene condivisa con gli altri attori, ed una chiave privata, che deve rimanere segreta.
\paragraph{Confidenzialità} 
La chiave pubblica del destinatario permette al mittente di cifrare il proprio messaggio, successivamente decifrato dal destinatario attraverso la propria chiave privata.
Questo meccanismo garantisce la confidenzialità del messaggio, in quanto solamente il destinatario è in grado di decifrare il messaggio cifrato.
\paragraph{Integrità}
...
Il mittente, per assicurarsi che il messaggio non venga alterato, associa al messaggio cifrato la funzione di hash applicata allo stesso. Ciò permette di garantire l'integrità del messaggio. 
\paragraph{Autenticità}
Sebbene, per garantire la proprietà di autenticità, il mittente cifra anche il risultato della funzione di hash, ottenendo la firma. Quando il destinatario decifra la firma, la confronta con il risultato dalla funzione di hash calcolata sul messaggio cifrato. Se i due valori coincidono, il destinatario è in grado di verificare che il messaggio non sia stato alterato e che provenga effettivamente dal mittente.
...
\paragraph{Tag}
Il MAC è un meccanismo attraverso il quale, dato un messaggio ed una chiave, forgia un tag, che inserisce insieme al messaggio da inviare. Il destinatario, una volta ricevuto il messaggio, calcola il tag associato, e lo confronta con quello ricevuto. Se i due tag coincidono, vi la certezza che il messaggio non sia stato alterato.
\paragraph{Funzione di hash}    
Una funzione di hash è una one-way function, che determina un valore di lunghezza fissa a partire da un messaggio di lunghezza arbitraria. Con one-way function si intende una funzione che non è computazionalmente possibile invertire, ovvero non è possibile risalire al messaggio a partire dal valore di hash. 
Questa funzione, deterministica, deve rispettare le seguenti proprietà:
\begin{itemize}
    \item \textbf{Non reversibilità}: data una funzione di hash $h$, non è computazionalmente possibile trovare un messaggio $m$ tale che $h(m)=h(m')$.
    ...
    \item \textbf{Resistenza alle collisioni}: non è computazionalmente possibile trovare due messaggi distinti $m_1$ e $m_2$ tali che $h(m_1)=h(m_2)$.
    \item \textbf{Hiding}: scelto un segreto  
\end{itemize}

\subsubsection{Certificato digitale} 
Un certificato è un documento elettronico che associa una chiave pubblica ad un'identità, e viene firmato digitalmente dalla CA. Nel documento sono presenti informazioni riguardanti l'università e la sua chiave pubblica, il CA e la sua firma, e il periodo di validità del certificato. 
